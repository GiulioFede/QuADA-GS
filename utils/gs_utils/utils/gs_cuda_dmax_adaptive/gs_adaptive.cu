...
__global__ void _gs_render_cuda(
    ...
    const int w,
    const int c,
    const float *dmax // <--- Cambiato in float*
) {
    int curs = blockIdx.x*blockDim.x + threadIdx.x;
    if(curs >= s) return;

    // Ogni thread prende il dmax della sua gaussiana
    float cur_dmax = dmax[curs]; 
    ...
    float r = colors[curs*3+0];
    float g = colors[curs*3+1];
    float b = colors[curs*3+2];

    float negative_half_one_div_one_minus_rho2 = -0.5 / (1.0 - rho*rho);
    float one_div_sigma_x_2 = 1.0 / (sigma_x * sigma_x);
    float one_div_sigma_y_2 = 1.0 / (sigma_y * sigma_y);
    float two_rho_div_sigma_x_one_div_sigma_y = 2.0 * rho / (sigma_x * sigma_y);

    for(int hi=0; hi<h; hi++){
        float curh_f = 2.0*hi/(h-1) - 1.0;
        float d_y = curh_f - y;
        if(d_y > cur_dmax || d_y < -cur_dmax) continue;

        for(int wi=0; wi<w; wi++){
            float curw_f = 2.0*wi/(w-1) - 1.0;
            float d_x = curw_f - x;
            if(d_x > cur_dmax || d_x < -cur_dmax) continue;

            float v = one_div_sigma_x_2*d_x*d_x - two_rho_div_sigma_x_one_div_sigma_y*d_x*d_y + one_div_sigma_y_2*d_y*d_y;
            v = exp(v * negative_half_one_div_one_minus_rho2);
...
void _gs_render(const float *sigmas, const float *coords, const float *colors, float *rendered_img, const int s, const int h, const int w, const int c, const float *dmax) {
    int threads=64;
...
__global__ void _gs_render_backward_cuda(
    const float *sigmas, const float *coords, const float *colors, const float *grads,
    float *grads_sigmas, float *grads_coords, float *grads_colors,
    const int s, const int h, const int w, const int c, const float *dmax
) {
    int curs = blockIdx.x*blockDim.x + threadIdx.x;
    if(curs >= s) return;

    float cur_dmax = dmax[curs]; // Legge il dmax specifico
    float sigma_x = sigmas[curs*3+0];
    float sigma_y = sigmas[curs*3+1];
    float rho = sigmas[curs*3+2];
    float x = coords[curs*2+0];
    ...
    for(int hi = 0; hi < h; hi++){
        ...
        for(int wi=0; wi < w; wi++){
            ...
            float v_2_w1 = v*2*w1;
            float g_vst_to_gsx = v_2_w1*(-w2*d_x+rho*w3*d_y);
            float g_vst_to_gsy = v_2_w1*(-w4*d_y+rho*w3*d_x);
...
void _gs_render_backward(const float *sigmas, const float *coords, const float *colors, const float *grads, float *grads_sigmas, float *grads_coords, float *grads_colors, const int s, const int h, const int w, const int c, const float *dmax) {
    int threads=64;
    dim3 grid(s, 1);
